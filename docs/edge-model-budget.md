...
| **argmax 一致率** | 60.11% | **97.75%**（W4A32） | 97.57% |
| decode（最佳内核） | 49.1 tok/s | **46.7 tok/s**（`sdot5_mt`，88.95%）<br>16.3 tok/s（`neon_mt`，97.75%） | 32.6 tok/s |
...
**新瓶颈是 W4A8 的激活量化。** RTN 时代 W4A8 与 W4A32 只差 1.9 点，
QAT 下差 8.8 点 —— 因为权重误差已被 QAT 压到近零，激活的 int8 量化
反而成了主误差源。但 88.95% 在文本上看不出劣化：两个内核的输出语义完全相同，
差别只在 markdown 加粗范围和一个「它」字（inventory §8.3）。
⇒ argmax 一致率会系统性过度惩罚格式层面的近平局。

**即：端侧上线不再卡在量化算法。** 剩下的取舍是速度/质量的内核选择
（`neon_mt` 97.75% @16.3 tok/s vs `sdot5_mt` 88.95% @46.7 tok/s），
以及 inventory §9 列的三条遗留项（W4A32 内核未优化、W4A8 激活精度、
unsloth UD-Q4_K_XL）。引擎侧（格式、导出、绑定、前向、KV 共享、
