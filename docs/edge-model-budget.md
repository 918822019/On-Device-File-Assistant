...
**RTN + group_size=64 对这个模型太粗。** 改进方向（按预期收益排序，均未实施）：

1. **HQQ 替代 RTN**：`write_tqwen_i4` 已支持 `--quant-method hqq`，
   但需第三方 `hqq` 包，本机未装。
2. **group_size 降到 32**：bpw 4.5 → 4.75（体积 +5.6%），最便宜的一步，应先试。
3. **敏感层保留 f16**：`classify` 回调已是逐张量的，改分类规则即可。
   需先定位误差来源（逐层单独量化、其余保持 f16，看一致率恢复多少）。
4. GPTQ/AWQ：需校准数据与 Hessian，本仓无实现，成本最高。

改完必须**同时**重跑两项：argmax 一致率（对 HF bf16）+ **有效语料**上的 PPL。
只跑 PPL 会被上面那个假象骗过去。
