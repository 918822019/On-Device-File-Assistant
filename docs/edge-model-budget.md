...
⚠️ **i4 的 decode 速度目前测不准，故不报数字。** `--matvec-impl` 只设一个
名字，而 i4 与 f16 是**两个独立注册表**：gemma4 的 i4 文件里 matmul 是 i4、
embed/lm_head 是 f16，选 `sdot4_mt` 时 i4 阶梯命中、f16 阶梯查不到就
**回退标量 ref**。lm_head 是 805 MB，标量跑约 330 ms/token，正好等于观测到的
decode 每步开销。两个阶梯唯一公共名字是 `ref`，故当前无法同时优化。
修法见 tiny-llm `9e5c72b` 的 commit message。
...
1. **matvec 阶梯的混合 dtype 选择**（上面那条），修完才能测 i4 的 decode 速度。
2. **PPL 门槛未测**。用户定的验收判据是「PPL 退化幅度 + greedy token 一致率」，
   目前只做了格式一致性的决定性验证（`verify_i4_roundtrip.py`：Python pack vs
   C++ unpack 差 5.96e-08、argmax 15/15，量化误差被完全对消），
   **量化质量本身还没量化过**。需要：C++ `--ppl` 在固定语料上跑
   f16 与 i4 各一遍，报相对退化；再跑长序列（≥256 token）的
   i4-vs-f16 greedy token 一致率。
