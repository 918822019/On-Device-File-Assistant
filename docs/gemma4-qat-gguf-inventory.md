...
### 8.6 宽语料复测（2152 token / 8 条 prompt）—— 结论加强，但「质量保证」仍未测

§8.5 的 96.07% 是在 534 token、2 条中文技术 prompt 的窄语料上测的，
分母只有 533 个位置（约 21 个不一致），样本量撑不起强结论。
故换成 **2152 token、8 条 prompt、807 个唯一 token**（中文技术 / 英文技术 /
数学应用题 / Python 代码 / 中译英）重测，并**在同一份语料上现测精度地板**：

| 模型 | 参考 | argmax 一致率 | margin 中位 |
|---|---|---|---|
| f16（原始权重） | HF-**原始** bf16 | **98.37%** ← 精度地板 | 4.93 |
| rtn-i4（RTN gs=64, sdot4_mt） | HF-原始 bf16 | 76.30% | 4.22 |
| **qat-`sdot6_mt`** | HF-**QAT** bf16 | **97.82%** | 2.91 |
| qat-`neon_mt`（W4A32 精确） | HF-QAT bf16 | 98.56% | 2.94 |
| f16（原始权重） | HF-**QAT** bf16 | 80.16% | 4.93 ← QAT 漂移量 |

**QAT + `sdot6_mt` 只比精度地板低 0.55 点，而 RTN i4 低 22.07 点 —— 损失小 40 倍。**

两个附带发现：
1. **宽语料上一致率反而更高**（sdot6 96.07→97.82、neon 97.75→98.56），
   说明窄语料里「格式/近平局位置」占比偏高。⇒ 小语料的一致率会**低估**质量。
2. **地板值本身随语料变**：README 语料 98.83%、窄语料 97.57%、宽语料 98.37%。
   ⇒ 任何一致率数字都必须与**同一份语料上现测的地板**比，
   **不能跨语料引用常数**（`eval_i4_quality.py` 里 `F16_BASELINE_AGREEMENT=98.83`
   只是个提示值，不是可跨实验套用的判据）。

⚠️ **但这组数字证明的是「量化不再损失质量」，不是「质量有保证」。**
97.82% 量的是**引擎+量化对 Google QAT 模型的复现保真度**，
不是**该模型在任务上有多好** —— 后者本项目从头到尾一次都没测过。仍未覆盖：

1. **任务级准确率**。argmax 一致率是代理指标；模型可以 100% 忠实地复现一个
   答错的答案。要下「质量有保证」的结论需要任务级评测（本项目没有）。
2. **长上下文（>2152 token）、多轮、非 greedy 采样** —— 全部测量都是短 greedy。
3. **真机**。`matvec_i4_sdot6.cpp` 被 `#if __aarch64__ && __ARM_FEATURE_DOTPROD`
   包着，设备上若无 dotprod 则整个内核不注册（此时 `--matvec-impl sdot6_mt`
   会 fail-loud 退出并列出可用名，不会静默降级 —— 这点是好的）。
4. **Google 这个 QAT 模型本身好不好**。上表最后一行的 80.16% 说明 QAT 把模型
   改动很大（相对原始模型有约 20% 的 argmax 位移），这笔交易划不划算
   我们**没有验证，是继承的信任**。

### 8.7 PLE 表压缩：f16 → i4，磁盘 −48%，一致率 −0.89 点（#32）

**动机不是内存。** PLE 表整表留盘（`--ple-ssd`）、每 token pread 一行，
**常驻内存为 0**，所以压缩它省的是**磁盘体积**与**每 token 的 I/O 量**，
1906 MB 常驻一分不变。这两项恰好都是手机端的紧约束（push 6.24 GiB、
以及 §8.6 里「PLE pread 在 UFS/eMMC 上的延迟未测」）。

`gguf_to_tiny_gemma4.py` 加 `--ple-dtype {f16,i4}`（默认 f16）。
选 i4 时走 `q6_k_to_i4_stream`：Q6_K → f32 → **非对称 RTN i4**（gs=32）。

⚠️ 这是**再量化，不是转码**。与 `q4_0_to_i4`（纯字节重排、逐位无损）性质
完全不同：Q6_K 的 6-bit 值与它训出来的子块 scale 都会被压成 4-bit + 我们
自己的 per-group scale，**两层误差叠加**。所以 `--verify` 对这条路径
不能再用「与 `dequantize` 逐位一致」当判据，改成报告相对误差
（实测抽样行最大相对 L2 = **8.58%**，RTN 4-bit 的典型量级约 9.6%）。

用**非对称** RTN：PLE 走 `dequant_i4_row` 查表路径、不经过 matvec 内核，
所以不受 `sdot5/sdot6` 的对称性约束，而非对称对非居中分布更准。

embed（`token_embd`）**不跟着压**：它是 tied lm_head，既当输入查表又当输出
投影，量化会同时打击两边的分布；而且它是**常驻**的（805 MB），压它省的是
内存而不是磁盘，是另一个取舍。`--ple-dtype` 只作用于 PLE 表。

**实测**（同一份 2152-token 宽语料、同一个 QAT bf16 参考，只有 PLE dtype 不同）：

| PLE dtype | 内核 | argmax 一致率 | margin 中位 |
|---|---|---|---|
| f16 | `sdot6_mt` | **97.82%** | 2.91 |
| **i4** | `sdot6_mt` | **96.93%** | 2.96 |
| i4 | `neon_mt` | 97.12% | 2.91 |

**代价 0.89 点**（判据是 ≤1 点 ⇒ 通过），距 98.37% 的精度地板 1.44 点，
仍远高于 95% 门槛。RTN 时代的消融测到的是 0.56 点，这次 0.89 略差，
符合「两层量化误差叠加」的预期。

| | PLE f16 | **PLE i4** | 变化 |
|---|---|---|---|
| 文件总字节 | 6,696,074,240（6.24 GiB） | **3,466,460,160（3.23 GiB）** | **−48%** |
| PLE 留盘 | 4480 MB | **1400 MB** | −69% |
| PLE 每行 pread | 17920 B | **5600 B** | −69% |
| 常驻权重 | 1905.87 MB | **1905.87 MB** | **不变** |
| decode（同一 prompt） | 20.41 ms/tok | **18.84 ms/tok** | 略快（I/O 少了） |
| argmax 一致率 | 97.82% | 96.93% | −0.89 点 |

⇒ **推荐把 PLE i4 版作为部署产物**，尤其手机端：push 量减半、每 token I/O 减 69%，
代价 0.89 个一致率点。f16 版保留作质量对照的上界。

⚠️ **一次差点记错的假结论。** 用 `tokenize_prompt.py --chat` 做文本对照时，
PLE-f16 版输出了「作为一个由阿里云通义千问（Qwen）开发的助手」的身份幻觉
且只讲思路不给代码，PLE-i4 版给出了正确的 `def is_palindrome(...)` ——
看起来像「量化后反而更好」。实际是那个工具的 `--system` 默认值硬编码了
Qwen 的身份提示，**模型是被告知的**；同时 token 数 17→38，与 eval 的语料
口径也不一致。改用正确模板后两者输出几乎逐字相同（只差 docstring 里一个
`(str)` 类型标注）。已修（默认改 `None`）并记为 AGENTS.md 坑 #47。
**教训：做 A/B 文本对照前先把两边的 prompt token 序列打出来对一遍。**
